	...
	void process_entry(trace_entry t)
	{
		...
		{	
			...
			if( l3_status == -1 )
			{
				...
				int l3_empty_status = check_l3_empty(addr_l3);										// Checking for empty space in L3
				if( l3_empty_status == -1 )
				{
					unsigned long long evicted_tag = L3_lru[addr_l3.setindex].evictLRU(); 			// Get LRU from L3
					l3_empty_status   = check_l3_cache(evicted_tag, addr_l3.setindex);  			// l3_empty_status = emptied position
				}
				L3_cache_update(addr_l3, l3_empty_status);											// Update L3 cache with new tag in emptied position
			...
			int l2_empty_status = check_l2_empty(addr_l2);											// In case of miss and hit , block is added to l2 ,check for empty block
												
			if(l2_empty_status == -1)	
			{
				unsigned long long evicted_tag = L2_lru[addr_l2.setindex].evictLRU();				// Evict LRU block
				l2_empty_status = check_l2_cache(evicted_tag, addr_l2.setindex);					// l2_empty_block = emptied location
			}

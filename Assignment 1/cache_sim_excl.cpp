...
public:
    ...
    LRU()
    { }
    void updateLRU( unsigned long long int tag)
    {
        tags.remove(tag);
        tags.push_front(tag);
    }
...
class cache_sim
{
	...
	int l2_block_process(split_addr addr_l2, split_addr addr_l3)
	{
		int l2_empty_pos = check_l2_empty(addr_l2);
		if( l2_empty_pos > -1)
		{
			return l2_empty_pos;
		}
		...
		{
			unsigned long long evict_l2_tag = L2_lru[addr_l2.setindex].evictLRU();
			l2_empty_pos = check_l2_cache(evict_l2_tag , addr_l2.setindex);

			split_addr evict_l2_tag_l3_eq =  addr_translate_l2_l3(evict_l2_tag, addr_l2.setindex);
			
			int l3_empty_pos = check_l3_empty(evict_l2_tag_l3_eq);
			if(l3_empty_pos > -1)
			{
				L3_cache_update(evict_l2_tag_l3_eq,l3_empty_pos);
			}
			else
			{
				unsigned long long evict_l3_tag = L3_lru[evict_l2_tag_l3_eq.setindex].evictLRU();
				l3_empty_pos = check_l3_cache(evict_l3_tag, evict_l2_tag_l3_eq.setindex);
				L3_cache_update(evict_l2_tag_l3_eq, l3_empty_pos);
			}			
			return l2_empty_pos;
		}
...
public:
	...
	void process_entry(trace_entry t)
	{
		...
		else
		{
			...
			if(l3_status > -1)
			{
				L3_hit++;															//L3 hit , invalidate block in L3 and copying to L2
				L3_invalidate(addr_l3.tags,addr_l3.setindex);
				int l2_empty_pos = l2_block_process(addr_l2, addr_l3);
				L2_cache_update(addr_l2,l2_empty_pos);
				
			}
			else
			{
				L3_miss++;															// L3 miss , copying block from memory to l2
				int l2_empty_pos = l2_block_process(addr_l2, addr_l3);
				L2_cache_update(addr_l2,l2_empty_pos);
			}
